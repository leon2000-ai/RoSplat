...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
                 image_name, uid,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device = "cuda", objects=None, depths=None, contours=None
                 ):
        ...
        if depths is not None:
            self.depths = depths.to(self.data_device)
        else:
            self.depths = None

        if contours is not None:
            self.contours = contours.to(self.data_device)
        else:
            self.contours = None
