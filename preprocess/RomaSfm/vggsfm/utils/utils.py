...
def average_batch_rotation_matrices(batch_rotation_matrices):
    ...
    # 5. 验证绕Z轴旋转特性
    # if not np.allclose(quaternions[:, :, 0:2], 0, atol=1e-6):
        # print("Warning: Not all rotations are strictly around Z-axis")
    
    # 6. 统一四元数符号
    # negative_w_mask = quaternions[:, :, 3] < 0
...
def generate_rank_by_dino(
    ...
    frame_feat0 = frame_feat = camera_predictor.backbone0(rgbs, is_training=True)
    frame_feat = camera_predictor.backbone.encoder(rgbs,upsample=False)
    using_feature = frame_feat[4]
    b,c,h,w = using_feature.shape
    using_feature = using_feature.reshape(b,c,h*w).permute(0,2,1)  # b * (h** c
    using_feature_norm = F.normalize(using_feature,p=2,dim=-1)

    # 
    frame_feat = {"x_norm_patchtokens":using_feature_norm,"x_norm_clstoken":None}


    frame_feat_global = frame_feat0["x_norm_clstoken"] 

    # 计算全局特征 (使用 [CLS] token 或平均) 
    # if frame_feat["x_norm_clstoken"] is not None:
    #     # 使用 [CLS] token
    #     frame_feat_global = frame_feat["x_norm_clstoken"]
    # else:
    #     # 使用全局平均
    #     pool = torch.nn.AdaptiveAvgPool2d((1,1))
    #     frame_feat_global = pool(frame_feat["x_norm_patchtokens"]).squeeze()
        
    #     print(f'global shape:{frame_feat_global.shape}') 

    frame_feat_norm = frame_feat['x_norm_patchtokens']
    ...
    frame_feat_norm = frame_feat_norm.permute(1, 0, 2)  # lbc * lcb
