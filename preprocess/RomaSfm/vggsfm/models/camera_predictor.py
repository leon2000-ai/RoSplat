...
class CameraPredictor(nn.Module):
    def __init__(
        ...
        self.backbone0 = self.get_backbone(backbone0)
    ...
    def get_2D_image_features(self, reshaped_image, batch_size):
        ...
        with torch.no_grad():
            ...
            frame_feat = self.backbone.encoder(reshaped_image,upsample=False)
            using_feature4 = frame_feat[4]
            b,c4,h4,w4 = using_feature4.shape
            

            using_feature8 = frame_feat[8]
            b,c8,h8,w8 = using_feature8.shape
           
            using_feature4 = F.adaptive_avg_pool2d(using_feature4, (h8, w8))

            using_feature = torch.cat([using_feature4, using_feature8], dim=1) # b c h8 w8
            b,c,h,w = using_feature.shape
            using_feature = using_feature.reshape(b,c,h*w).permute(0,2,1)  # b * (h*w) * c
            using_feature_norm = F.normalize(using_feature,p=2,dim=-1)

            rgb_feat = {"x_norm_patchtokens":using_feature_norm,"x_norm_clstoken":None}

            ###############

            # B x P x C
            rgb_feat = rgb_feat["x_norm_patchtokens"].float()
