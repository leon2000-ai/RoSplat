...
class VGGSfMRunner:
    ...
    def sparse_reconstruct(
        ...
        # # 在 sparse_reconstruct 方法末尾添加
        # if self.cfg.save_to_disk:
        #     # 计算并输出重投影误差
        #     error_stats = calculate_reprojection_error(reconstruction)
            
        #     # 输出到日志
        #     logger.info("Reprojection Error Statistics:")
        #     for key, value in error_stats.items():
        #         logger.info(f"  {key}: {value}")
            
        #     # 保存到文件
        #     error_file = os.path.join(output_dir, "reprojection_errors.txt")
        #     with open(error_file, 'w') as f:
        #         for key, value in error_stats.items():
        #             f.write(f"{key}: {value}\n")
            
        #     logger.info(f"Saved reprojection error statistics to {error_file}")
