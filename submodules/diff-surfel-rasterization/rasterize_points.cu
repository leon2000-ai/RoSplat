	...
	const bool debug)
{
  ...
  torch::Tensor max_contrib_gs_id = torch::full({H, W}, -1, int_opts);  // gs_id tracking
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		max_contrib_gs_id.contiguous().data<int>(),  // pass gs_id
		debug);
  }
  return std::make_tuple(rendered, out_color, out_others, radii, max_contrib_gs_id, geomBuffer, binningBuffer, imgBuffer);
}
